...
判断依据：**带位置编号参数（版本号 N）的 skill 一律不许自动触发。** 模型自行触发时拿不准该用哪个版本号，猜错就会往错误的版本目录里写东西。`xz-plan`、`xz-update-plan`、`xz-exec`、`xz-worktree` 都是因此禁掉的——后两个还会**真的改源码**，误触发的代价比写错一份计划更大（`xz-exec` 一度是例外，2026-09-10 起收回）。
