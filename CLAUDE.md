...
**只有 `xz-debug`、`xz-exec`、`xz-worktree` 允许模型自动触发，其余 16 个全部禁用。**

判断依据：**带位置编号参数（版本号 N）的 skill 默认不许自动触发。** 模型自行触发时拿不准该用哪个版本号，猜错就会往错误的版本目录里写东西。`xz-plan`、`xz-update-plan` 就是因此禁掉的。

**`xz-exec` 与 `xz-worktree` 是有意开的例外**（2026-09-10 定）：它们是最常走的执行入口，每次都要手敲太啰嗦。兜底是这两个 skill 开头都先 `parse N`，版本不存在就停下提示去跑 `/xz-plan N`。**但要清楚这个兜底挡不住什么**——它只能拦「版本不存在」，拦不住「猜了另一个确实存在的版本号」；那种情况下会按错误的计划改源码。这是明知的取舍，不是漏判。
