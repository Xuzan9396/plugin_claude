...
- **文档对齐代码**：凡最终结论为「等价保留」或「确认保留手动实现」的条目，必须同步改写 N-PLAN.md 的 change details（行号/现状摘要/改动点按实际代码更新）和 N-MANUAL.md 对应改动块。MANUAL 对齐以“修改已有文件统一使用 `========= v计划号 真实改动标题 before|start|end ===========`，新建文件给整文件，删除贴完整原文”为准：修改场景只回写实际改动块和最小必要定位上下文，不把未改方法整段带回；新增文件仍可保留整文件；若片段里出现函数/方法，顶部注释与案例说明也要按实际代码同步，保证三者一致：源码 = PLAN = MANUAL；源码本身一行不动
- 每条处理后更新 `> 最后更新:` 与变更记录
