...
该版本由 `/xz-plan` 的「手动执行」路径生成：用户已照 `N-MANUAL.md` 手动改完源码。当前 `N-MANUAL.md` 的默认格式是“仅核心改动使用 `========= v计划号 真实改动标题 before/start/end ===========`，普通小改动保持简洁写法”；只有新建文件才给整文件，修改文件默认不再给整文件。本次审核分级处理：一致/等价**自动勾选不打扰**；发现问题**只记录不动手**，全部审完后汇总一次性找用户确认——**用户手动改的代码，未经允许绝不改动**。对齐方向是**文档跟代码走**：手动实现等价但风格不同时，改 N-PLAN.md / N-MANUAL.md 去贴合手动代码，而不是反过来。
...
- **文档对齐代码**：凡最终结论为「等价保留」或「确认保留手动实现」的条目，必须同步改写 N-PLAN.md 的 change details（行号/现状摘要/改动点按实际代码更新）和 N-MANUAL.md 对应改动块。MANUAL 对齐以“仅核心改动使用 `========= v计划号 真实改动标题 before/start/end ===========`，普通小改动保持简洁写法”为准：修改场景只回写实际改动块和最小必要定位上下文，不把未改方法整段带回；新增文件仍可保留整文件；若片段里出现函数/方法，顶部注释与案例说明也要按实际代码同步，保证三者一致：源码 = PLAN = MANUAL；源码本身一行不动
- 每条处理后更新 `> 最后更新:` 与变更记录
...
12. **校验模式靠状态触发** — PLAN 头部 `状态: 待手动执行`（由 `/xz-plan` 手动路径写入）时进入校验模式：一致/等价直接勾选不打扰；问题点只记录不动手，审完汇总成编号清单（每项 a/b 处理选项）**一次性**找用户确认，**未经用户允许绝不改动其手动代码**；MANUAL 以“仅核心改动使用 `========= v计划号 真实改动标题 before/start/end ===========`，普通小改动保持简洁写法”为准，修改场景不要回写未改方法整段；等价/确认保留的条目**文档对齐代码**——同步改 N-PLAN.md 与 N-MANUAL.md 贴合手动实现，不改源码；其他状态走常规执行，行为不变
