...
**本 skill 目录：** 本 skill 的本地文件（agents、scripts、visual-companion）需要通过绝对路径访问。**执行前第零步**：运行

```bash
xz-tools.py skill-path xz-discuss
```

拿到返回的绝对路径（如 `/Users/xxx/.claude/plugins/cache/xz-tools/xz-planning/1.0.3/skills/xz-discuss`），**记为 `$SKILL_DIR`**，本文档后续所有出现的 `$SKILL_DIR` 字面量都替换成这个真实路径再使用（包括 Read 工具的文件路径和 Bash 命令）。
...
用 Bash 工具执行（`$SKILL_DIR` 用第零步拿到的真实绝对路径替换）：
