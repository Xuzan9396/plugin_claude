...
**卡住一定会喊你，不会闷头死磕**（防死循环护栏）：同一条用例最多修 2 次、全局最多 3 轮、同一失败最多重跑确认 2 次、单条命令超 2 分钟判挂起、同一问题最多追问 2 轮。触到上限或遇到下列情况就停下来找你——根因说不清、缺信息改法全靠猜、环境起不来、需要真实凭证/线上环境/真机硬件、**需要你手动操作到某一步**（验证码、扫码、后台点按钮）。求助时会把无关用例先跑完，再把卡壳项**攒成一份清单一次性问**，每项写清「现象 / 已尝试 / 卡在哪 / 需要你 / 拿到后我做什么」。你可以：直接给信息、回「N 已就绪」（你手动操作到位了，AI **从那个状态接着跑，不让你重做一遍**）、或回「跳过 N」。提问后到你回复前 AI 真的会停手，不改代码也不再试。
